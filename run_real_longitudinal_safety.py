...
from real_longitudinal_safety_runner import (
    InputHealthSnapshot, LiveFileSampleSource, RealLongitudinalSafetyRunner,
)
from realtime_bev_renderer import render_realtime_frame

FIELDS=("frame","timestamp_ns","gps_speed_mps","gps_valid","gps_reason","speed_source",
...
"critical_boundary_m","zone","safety_status","safe_target_available",
"coverage_status","nominal_target_speed_mph",
...
def format_input_health_summary(health: InputHealthSnapshot) -> str:
    return (
        f"Processed frames:         {health.processed_frames}\n"
        f"Skipped/unpaired frames:  {health.skipped_unpaired_frames}\n"
        f"Malformed sensor lines:   {health.malformed_sensor_lines}\n"
        f"Expired pairs:            {health.expired_pairs}\n"
        f"Last processed frame ID:  {health.last_processed_frame_id}"
    )

def main():
    ap=argparse.ArgumentParser(
        description="Read-only standalone consumer for live BEV + GPS/IMU output."
    )
    ap.add_argument("--matrix-dir",type=Path,default=Path("matrix"),
                    help="directory containing numbered 120x80 uint8 .npy files")
    ap.add_argument("--sensor-file",type=Path,default=Path("sensors/matrix_sensors.jsonl"),
                    help="append-only matrix_sensors.jsonl path")
    ap.add_argument("--start",default="latest",help="latest, earliest, or frame ID")
    ap.add_argument("--poll-ms",type=float,default=10.0,
                    help="idle input poll interval in milliseconds (default: 10)")
    ap.add_argument("--headless",action="store_true",
                    help="process and log without an OpenCV window")
    ap.add_argument("--log",type=Path,
                    help="explicit streamed CSV output path")
    ...
    stopped="Ctrl+C"
    last_warning_counts=(0,0)
    try:
        while True:
            ready=source.poll_ready(limit=32)
            health=source.health_snapshot(runner.processed_count)
            warning_counts=(health.skipped_unpaired_frames,health.malformed_sensor_lines)
            if warning_counts != last_warning_counts and health.warning:
                print(
                    "INPUT WARNING: "
                    f"skipped/expired pairs={warning_counts[0]}, "
                    f"malformed sensor lines={warning_counts[1]}"
                )
            last_warning_counts=warning_counts
            if not ready:
            ...
            for frame_id,path,matrix,record in ready:
                ...
                if not args.headless:
                    ...
                    cv2.imshow(
                        ...
                        render_realtime_frame(frame,stats(samples),health),
                    )
        ...
        print(format_input_health_summary(source.health_snapshot(runner.processed_count)))
